...
fn ensure_vst3_validator(ctx: &Context) -> Result<PathBuf> {
    ...
    // Patched for open-clang: only request the Xcode generator when the
    // platform actually resolves one (Command Line Tools-only machines use
    // the default Makefile generator, matching the plugin-build patches).
    if ctx.platform == Platform::Macos && ctx.platform.cmake_generator() == Some("Xcode") {
