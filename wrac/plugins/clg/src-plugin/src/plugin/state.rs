...
pub(super) struct ClgStateExtension {
    shared: Arc<SharedState>,
}

impl ClgStateExtension {
    pub(super) fn new(shared: Arc<SharedState>) -> Self {
        Self { shared }
    }
}

impl PluginStateExtension for ClgStateExtension {
    ...
    fn restore_state(&self, state: State) -> PluginResult<()> {
        ...
        for info in parameter_infos() {
            let _ = self.shared.set_parameter_value(info.id, info.default_value);
        }
